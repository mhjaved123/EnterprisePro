...
/* 'Add New Project' & 'Update Project Records' Form */
.form {
  border: 2px solid black;
  width: 1130px;
  height:120px;
...
<div class="navbar">
  ...
  <div class="main">
    <h2>Projects</h2>
    <p> View and edit project records </p>
    <br>
...
if (isset($_GET['update'])) {
  ...
  $update = true;
  $rec = mysqli_query($connection, "SELECT * FROM projects WHERE projectNo=$projectNo");
...
<!-- 'Add New Project' & 'Update Project Info' Form' -->
   ...
   <?php if ($update == false): ?>
    <h2 class="form-header"> Add a New Project: </h2>
  <?php else: ?>
    <h2 class="form-header"> Update Project Information: </h2>
    <?php endif ?>
    <div class="form-input">
      <label>Project ID: </label>
      <input type="number" name="projectNo" required value="<?php echo $projectNo; ?>">
   </div>
   <div class="form-input">
   ...
   <div class="form-input">
     <?php if ($update == false): ?>
      <button type="submit" name="add" class="form-btn">Add Project</button>
    <?php else: ?>
      ...
      <?php
       ...
       // connect to server address, username, password, then database 
        ...
        while($row=mysqli_fetch_array($results)) {
          ...
          <td>" . $row["sprvisor"] . "</td>
          <td>
            <a href='projects.php?update=". $row["projectNo"] . "' class='update-btn'>Update</a>
            <a href='projects-crud.php?del=" . $row["projectNo"] . "' class='delete-btn'>Delete</a>
        </tr>";
        }
        }
